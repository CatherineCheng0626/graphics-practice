...
function startup() {
  ...
  myTerrain = new Terrain(60, -1, 1, -1, 1);
  maxElevation = myTerrain.getMaxElevation();
  minElevation = myTerrain.getMinElevation();
...
var lookAtPt = glMatrix.vec3.fromValues(0.0, 0.0, -0.5);
var eyePt = glMatrix.vec3.fromValues(0.0, -1.75, 1.5);
var up = glMatrix.vec3.fromValues(0.0, 1.0, 0.0);
var viewDir = glMatrix.vec3.fromValues(0.0, 1.0, -1.0);

/**
 * Draws the terrain to the screen.
 */
function draw() {
  var tmp = glMatrix.vec4.fromValues(lightPosition[0],lightPosition[1], lightPosition[2], 1.0);
  glMatrix.vec4.multiply(tmp, tmp, modelViewMatrix);
  lightPosition = glMatrix.vec3.fromValues(tmp[0], tmp[1], tmp[2]);
...
var camSpeed = 0.001;                                 //the camera's current speed in the forward direction
var pitch = glMatrix.vec3.create();
 ...
 */
 function animate(currentTime) {
  if (keys["Escape"]) {
    camSpeed = 0.001;
    lookAtPt = glMatrix.vec3.fromValues(0.0, 0.0, -0.5);
    eyePt = glMatrix.vec3.fromValues(0.0, -1.75, 1.5);
    up = glMatrix.vec3.fromValues(0.0, 1.0, 0.0);
    viewDir = glMatrix.vec3.fromValues(0.0, 1.0, -1.0); 
  }
  if (keys["="]) {
    camSpeed += 0.0004;
  }
  ...
  if (keys["ArrowUp"]) {
    glMatrix.vec3.cross(pitch, viewDir, up);
    glMatrix.quat.setAxisAngle(camOrientation, pitch, degToRad(1));
    glMatrix.vec3.transformQuat(viewDir, viewDir, camOrientation);
  ...
  if (keys["ArrowLeft"]) {
    glMatrix.quat.setAxisAngle(camOrientation, viewDir, degToRad(1));
    glMatrix.vec3.transformQuat(viewDir, viewDir, camOrientation);
    glMatrix.vec3.transformQuat(up, up, camOrientation);
  }
  if (keys["ArrowRight"]) {
  ...
  glMatrix.vec3.scale(movement, viewDir, camSpeed);
