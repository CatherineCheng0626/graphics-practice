   ...
   */
    ...
    */
    ...
    {    
        ...
        for (var line of lines) {
            const word = line.split(/\b\s+(?!$)/);
            if (word[0] === 'v') {
                this.vBuffer.push(parseFloat(word[1]));
                this.vBuffer.push(parseFloat(word[2]));
                this.vBuffer.push(parseFloat(word[3]));
            } else if (word[0] === 'f') {
                this.fBuffer.push(parseInt(word[1]) - 1);
                this.fBuffer.push(parseInt(word[2]) - 1);
                this.fBuffer.push(parseInt(word[3]) - 1);
            } else {
                continue;
            }
        }
    ...
    */    
    ...
    canonicalTransform(){
      //Your code here
      this.computeAABB();
      var Tx = (this.maxXYZ[0] + this.minXYZ[0]) / -2;
      var Ty = (this.maxXYZ[1] + this.minXYZ[1]) / -2;
      var Tz = (this.maxXYZ[2] + this.minXYZ[2]) / -2;
      var T = glMatrix.vec3.fromValues(Tx, Ty, Tz);

      var L = Math.max(Math.max(this.maxXYZ[0] - this.minXYZ[0], this.maxXYZ[1] - this.minXYZ[1]), this.maxXYZ[2] - this.minXYZ[2]);
      var S = glMatrix.vec3.fromValues(1/L, 1/L, 1/L);
